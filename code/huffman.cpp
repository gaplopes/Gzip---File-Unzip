...
// procura c�digo na �rvore a partir da ra�z (representado por String com 0s e 1s): procura codigo inteiro
int findNode(HuffmanTree *hft, char *s, short verbose) {
  return findNode(hft, s, hft->root, verbose);
}

// procura codigo na �rvore, a partir do n� actual (representado por String com 0s e 1s)
// devolve �ndice no alfabeto, se encontrou; -1 se n�o encontrou; -2 se � prefixo de codigo existente;
int findNode(HuffmanTree *hft, char *s, HFNode *cur, short verbose) {
  ...
  if (verbose) {
    if (pos == -1)
      printf("Codigo '%s' n�o encontrado!!!\n", s);
    else if (pos == -2)
      printf("Cdigo '%s' n�o encontrado, mas prefixo!!!\n", s);
    else
      printf("Cdigo '%s' corresponde � posi��o %d do alfabeto\n", s, pos);
  }
